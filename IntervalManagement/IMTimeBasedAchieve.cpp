...
IMTimeBasedAchieve::IMTimeBasedAchieve()
      : m_target_state_at_traffic_alignment(),
        ...
        m_target_dtg_at_cdti_initiate_signal_receipt(Units::Infinity()),
...
void IMTimeBasedAchieve::Copy(const IMTimeBasedAchieve &obj) {
   ...
   m_target_dtg_at_cdti_initiate_signal_receipt = obj.m_target_dtg_at_cdti_initiate_signal_receipt;
   ...
   m_cdti_initiate_signal_receipt_time = obj.m_cdti_initiate_signal_receipt_time;
}
...
Guidance IMTimeBasedAchieve::Update(const Guidance &previous_im_guidance,
   ...
   if (m_im_clearance.GetClearanceType() == IMClearance::MAINTAIN) {
      if (m_cdti_initiate_signal_receipt_time == Units::Infinity() && !target_adsb_history.empty()) {
         ...
         m_im_ownship_distance_calculator.CalculateAlongPathDistanceFromPosition(
               ...
               m_target_dtg_at_cdti_initiate_signal_receipt);
      }
   }
   ...
   if (!IsImOperationComplete() && guidance_out.IsValid()) {
      ...
      if (InAchieveStage()) {
         return HandleAchieveStage(current_ownship_state,
                                   current_target_state,
                                   target_adsb_history,
                                   three_dof_dynamics_state,
                                   guidance_out);
      } else {
         ...
         if (m_im_clearance.GetClearanceType() == IMClearance::CAPTURE) {
            if (!IsTargetAligned()) {
               ...
               return HandleMaintainStage(current_ownship_state,
                                          current_target_state,
                                          target_adsb_history,
                                          three_dof_dynamics_state,
   ...
   if (m_target_aircraft_exists && !IsTargetPassedTrp()) {
      ...
      if (m_target_reference_lookup_index == 0) {
         ...
         m_target_reference_ias = Units::MetersPerSecondSpeed(
               m_target_kinematic_trajectory_predictor.GetVerticalPathVelocityByIndex(0));
...
Guidance IMTimeBasedAchieve::HandleMaintainStage(const AircraftState &current_ownship_state,
   ...
   if (built) {
      ...
      guidance_out = m_im_kinematic_time_based_maintain->Update(three_dof_dynamics_state,
      ...
      if (!HasOwnshipReachedTargetAlongPathPositionAtCdtiInitiation()) {
         ...
         return time_since_cdti_initiate_signal_receipt +
                ((m_ownship_kinematic_dtg_to_ptp - m_target_dtg_at_cdti_initiate_signal_receipt) /
                 m_target_state_at_cdti_initiate_signal_receipt.GetGroundSpeed());
      }
   }
