...
void IMUtils::CalculateTimeBasedExtrapolate(const Units::Length &ownship_dtg,
  ...
  extrapolated_target_time =
      Units::SecondsTime(oldest_target_state.GetTimeStamp().value()) - delta_time; // time when target was at dtg_ownship
  interval_management::AircraftState extrapstate, projstate(oldest_target_state);
...
/**
 * An algorithm that calculates an extrapolated time for a provided distance-to-go. Uses constant ground speed as an assumption
 * since only one state is provided.  Uses AlongPathDistanceCalculator for projected position.
 *
 * @param ownship_dtg: the distance-to-go on ownship's route
 * @param oldest_target_state: the target state to use for the extrapolation
 * @param ownship_horizontal_traj: ownship's horizontal path
 ...
 */
void IMUtils::GetTimeBasedExtrapolateState(const interval_management::AircraftState &current_ownship_state,
                                           const interval_management::AircraftState &oldest_target_state,
                                           const std::vector<HorizontalPath> &ownship_horizontal_traj,
  ...
  CalculateTimeBasedExtrapolate(ownship_dtg,
                                ...
                                projDTG);
  Units::Time msi =
      Units::SecondsTime(current_ownship_state.GetTimeStamp().value()) - extrapolated_target_time; // the spacing interval in time
  ...
  aircraft_state_to_return.Create(
        oldest_target_state.GetId(),
        extrapolated_target_time,
        extrapolated_position,
        oldest_target_state.GetSpeedXd(),
        oldest_target_state.GetSpeedYd(),
        oldest_target_state.GetSpeedZd(),
        Units::zero(),
        Units::zero(),
        Units::zero(),
        Units::zero(),
...
bool IMUtils::GetCrossingTime(const Units::Length current_dtg,
  ...
  if (index > 0 && found) {
    ...
    crossing_time = Units::SecondsTime(
        (aircraft_state_history[index].GetTimeStamp().value() - aircraft_state_history[index - 1].GetTimeStamp().value())
    ...
    interval_management::AircraftState crossingstate, beforestate(aircraft_state_history[index - 1]), afterstate(
 ...
 */
bool IMUtils::CalculateTargetStateAtTime(const interval_management::AircraftState &target_state,
                                         const std::vector<HorizontalPath> &ownship_horizontal_traj,
                                         const Units::Time extrapolation_time,
                                         const Units::Angle ownship_true_heading,
                                         interval_management::AircraftState &extrapolation_state) {

  interval_management::AircraftState projstate(target_state), stateattime;
  ...
  Units::Time dt =
      Units::SecondsTime(target_state.GetTimeStamp().value()) - extrapolation_time; // sign is important, pos means going backwards
  ...
  extrapolation_state.Create(
        target_state.GetId(),
        extrapolation_time,
        extrapolated_position,
        target_state.GetGroundSpeed() * Units::cos(crs),
        target_state.GetGroundSpeed() * Units::sin(crs),
        Units::zero(),
        Units::zero(),
        Units::zero(),
        Units::zero(),
...
interval_management::AircraftState IMUtils::
GetTargetStateOnOwnshipPathForDtg(const vector<interval_management::AircraftState> &target_adsb_history,
                                  const std::vector<HorizontalPath> &ownship_horizontal_path,
                                  const Units::Length target_distance,
                                  const Units::Angle ownship_true_heading) {
  interval_management::AircraftState result;
  interval_management::AircraftState target_state_to_extrapolate_from;
  ...
  if (ix != 0 && ix < target_adsb_history.size()) {
    interval_management::AircraftState before = target_adsb_history[ix - 1];
    interval_management::AircraftState after = target_adsb_history[ix];
    ...
    if (b0 && b1) {
      const Units::Time target_time_at_dtg = IMUtils::InterpolateTimeAtDtg(target_distance,
                                                                           Units::SecondsTime(before.GetTimeStamp().value()),
 ...
 */
void IMUtils::CalculateTargetStateAtDistance(const interval_management::AircraftState &target_state,
                                             const std::vector<HorizontalPath> &ownship_horizontal_traj,
                                             const Units::Length extrapolation_distance,
                                             const Units::Angle ownship_true_heading,
                                             interval_management::AircraftState &extrapstate) {

  interval_management::AircraftState projstate(target_state);
  interval_management::AircraftState stateattime;
  Units::FeetLength projx, projy, projdtg;
...
interval_management::AircraftState IMUtils::GetProjectedTargetState(const std::vector<interval_management::AircraftState> &target_state_history,
                                               const std::vector<HorizontalPath> &ownship_horizontal_traj,
                                               const Units::Time target_time,
                                               const Units::Angle ownship_true_heading,
                                               bool &target_state_is_valid) {
  interval_management::AircraftState target_state;
  interval_management::AircraftState extrapolateFromThis;
  ...
  if (ix != 0 && ix < target_state_history.size()) {
    interval_management::AircraftState before(target_state_history[ix - 1]), after(target_state_history[ix]);
    Units::Length beforex, beforey, beforedtg;
    Units::Length afterx, aftery, afterdtg;
    const bool b0 = IMUtils::ProjectTargetPosition(
...
interval_management::AircraftState IMUtils::GetProjectedTargetState(AlongPathDistanceCalculator &distance_calculator,
...
*/
    ...
    if (is_extrapolation_valid) {
      ...
      target_state.Create(
        extrapolateFromThis.GetId(),
        Units::SecondsTime(target_time),
        extrapolated_position,
        extrapolateFromThis.GetGroundSpeed() * Units::cos(dummyangle),
        extrapolateFromThis.GetGroundSpeed() * Units::sin(dummyangle),
        Units::zero(),
        Units::zero(),
        Units::zero(),
        Units::zero(),
...
interval_management::AircraftState IMUtils::ConvertToIntervalManagementAircraftState(const aaesim::open_source::AircraftState &aircraft_state) {
   interval_management::AircraftState new_state;
   return new_state.Create(
      aircraft_state.m_id,
      Units::SecondsTime(aircraft_state.m_time),
      EarthModel::LocalPositionEnu::Of(aircraft_state.GetPositionX(), aircraft_state.GetPositionY(), aircraft_state.GetPositionZ()),
      aircraft_state.GetSpeedXd(),
      aircraft_state.GetSpeedYd(),
      aircraft_state.GetSpeedZd(),
      Units::RadiansAngle(aircraft_state.m_gamma),
      Units::MetersPerSecondSpeed(aircraft_state.m_Vwx),
      Units::MetersPerSecondSpeed(aircraft_state.m_Vwy),
      Units::MetersPerSecondSpeed(aircraft_state.m_Vw_para),
      Units::MetersPerSecondSpeed(aircraft_state.m_Vw_perp),
      aircraft_state.m_sensed_temperature,
      aircraft_state.m_psi
   );
}

aaesim::open_source::AircraftState IMUtils::ConvertToAaesimAircraftState(const interval_management::AircraftState &aircraft_state) {
