...
void TestVectorSpeedControl::InitializeFmsPredictors(
...
aaesim::open_source::Guidance TestVectorSpeedControl::Update(
   ...
   if (return_guidance.IsValid()) {
      ...
      // Output only
      {
         ...
         Units::Speed tas_command =
               m_standard_atmosphere.CAS2TAS(new_ias_command, Units::FeetLength(owntruthstate.m_z));
         Units::Length ownship_true_dtg;
         m_distance_calculator.CalculateAlongPathDistanceFromPosition(
               Units::FeetLength(owntruthstate.m_x), Units::FeetLength(owntruthstate.m_y), ownship_true_dtg);
         InternalObserver::getInstance()->IM_command_output(
               owntruthstate.GetId(), owntruthstate.GetTimeStamp().value(), owntruthstate.m_z,
               Units::MetersPerSecondSpeed(dynamicsstate.v_true_airspeed).value(),
               Units::MetersPerSecondSpeed(owntruthstate.GetGroundSpeed()).value(),
               Units::MetersPerSecondSpeed(new_ias_command).value(),
               Units::MetersPerSecondSpeed(new_delayed_ias_command).value(),
               Units::MetersPerSecondSpeed(tas_command).value(), 0.0, 0.0,
...
const double TestVectorSpeedControl::GetSpacingError() const { return UNDEFINED_INTERVAL; }
