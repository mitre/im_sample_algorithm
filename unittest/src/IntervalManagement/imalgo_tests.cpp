...
class IMAchieveTest : public IMAchieve, public ::testing::Test {
   // A mock implementation that allows us to get at protected methods in
   // IMAchieve
  protected:
...
TEST_F(IMAchieveTest, SetMaxSpeedDeviationPercentage) {
   SetMaxSpeedDeviationPercentage(BoundedValue<double, 0, 100>(25));
   const double nominal3(3);
   EXPECT_DOUBLE_EQ(m_speed_limiter.HighLimit(nominal3), 3.75);
   EXPECT_DOUBLE_EQ(m_speed_limiter.LowLimit(nominal3), 2.25);
}
...
TEST_F(IMAchieveTest, errorThreshold) {
   ...
   // A negative slope would cause error threshold to increase as ownship
   // approaches ABP
...
TEST(IMAlgorithm, API) {
   auto im_time_based_achieve = std::make_shared<IMTimeBasedAchieve>();

   im_time_based_achieve->EnableDefaultPilotDelayModel();
   im_time_based_achieve->DisablePilotDelayModel();

   im_time_based_achieve->SetLimitFlag(true);
   ...
   im_time_based_achieve->GetOwnshipTtgtoPtp();
   im_time_based_achieve->GetOwnshipDtgtoAbp();
   im_time_based_achieve->GetFlightStage();
   im_time_based_achieve->GetTargetTtgToTrp();
   im_time_based_achieve->GetTargetTtgToEndOfRoute();
   im_time_based_achieve->GetSpeedChangeCount();
   im_time_based_achieve->GetUnmodifiedImSpeedCommandIas();
   im_time_based_achieve->GetPsi();
   im_time_based_achieve->GetMsi();
   im_time_based_achieve->GetAssignedSpacingGoal();
   im_time_based_achieve->GetActiveFilter();
   im_time_based_achieve->GetSpacingInterval();
   im_time_based_achieve->GetSpacingError();
   im_time_based_achieve->GetTargetKinematicDtgToTrp();
   im_time_based_achieve->GetTargetDtgToLastWaypoint();
   im_time_based_achieve->GetClearance();
   im_time_based_achieve->IsTargetPassedTrp();
   im_time_based_achieve->IsImOperationComplete();
   im_time_based_achieve->InAchieveStage();
   im_time_based_achieve->IsWithinErrorThreshold();
   im_time_based_achieve->GetOwnshipKinematicPredictor();
   im_time_based_achieve->GetTargetKinematicPredictor();

   im_time_based_achieve->GetClearance().AbpAndPtpAreColocated();
   im_time_based_achieve->GetClearance().IsCoincidentRoutePairing();

   const BoundedValue<double, 0, 100> test_deviation_percentage(50);
   im_time_based_achieve->SetMaxSpeedDeviationPercentage(test_deviation_percentage);

   im_time_based_achieve->SetRecordMaintainMetrics(false);
   ASSERT_FALSE(im_time_based_achieve->GetRecordMaintainMetrics());

   auto ownship_vertical_predictor = im_time_based_achieve->GetOwnshipVerticalPredictor();
   ASSERT_TRUE(ownship_vertical_predictor == nullptr);

   auto target_vertical_predictor = im_time_based_achieve->GetTargetVerticalPredictor();
   ASSERT_TRUE(target_vertical_predictor == nullptr);
   ASSERT_FALSE(im_time_based_achieve->IsNewTrajectoryPredictionAvailable());
}
...
TEST(IMUtils, projTargetPosition) {
   ...
   bool b1 = IMUtils::ProjectTargetPosition(target_x, target_y, ownship_horizontal_traj, xProjected, yProjected,
                                            dtgProjected);
   EXPECT_TRUE(b1);
   ...
   /*
    * Check again with a very large cross track error (make target_y big). Note
    * that cte can't be larger than 2.5 NM according to AircraftCalculations
    * algorithms.
    */
   const Units::Length target_y_large = target_x + Units::NauticalMilesLength(2.0);
   bool b2 = IMUtils::ProjectTargetPosition(target_x, target_y_large, ownship_horizontal_traj, xProjected, yProjected,
                                            dtgProjected);
   EXPECT_TRUE(b2);
   EXPECT_NEAR(xExpect.value(), xProjected.value(), tol.value());
   EXPECT_NEAR(yExpect.value(), yProjected.value(), tol.value());
   ...
   /*
    * Check again with a state off the back of the route and with minor cross
    * track error.
    */
   const Units::Length target_x_large = Units::MetersLength(hp.GetXPositionMeters()) + Units::NauticalMilesLength(2.0);
   bool b3 = IMUtils::ProjectTargetPosition(target_x_large, target_y, ownship_horizontal_traj, xProjected, yProjected,
                                            dtgProjected);
   EXPECT_TRUE(b3);
   ...
   /*
    * Check again with a state off the back of the route and with significant
    * cross track error.
    */
   const Units::Length large_cte = target_y + Units::NauticalMilesLength(3.0);
   bool b4 = IMUtils::ProjectTargetPosition(target_x_large, large_cte, ownship_horizontal_traj, xProjected, yProjected,
                                            dtgProjected);
   EXPECT_FALSE(b4);
...
TEST(IMUtils, projTargetState_simple) {
   ...
   interval_management::open_source::AircraftState asv;
   asv.Create(1000, Units::zero(),
              EarthModel::LocalPositionEnu::Of(Units::MetersLength(hp1.GetXPositionMeters()),
                                               Units::MetersLength(hp1.GetYPositionMeters()), Units::zero()),
              Units::FeetPerSecondSpeed(xd), Units::FeetPerSecondSpeed(yd), Units::zero(), Units::zero(), Units::zero(),
              Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero());

   Units::Length x, y, dtg;
   IMUtils::ProjectTargetPosition(asv.GetPositionX(), asv.GetPositionY(), ownship_horizontal_traj, x, y, dtg);
...
TEST(IMUtils, getCrossingTime) {
   ...
   vector<interval_management::open_source::AircraftState> states;

   // test with empty states vector (invalid). Case E in the algorithm
   valid = IMUtils::GetCrossingTime(Units::MetersLength(150), states, traj, crossingTime);
   EXPECT_FALSE(valid);
   interval_management::open_source::AircraftState s0;
   s0.Create(0, Units::SecondsTime(55),
             EarthModel::LocalPositionEnu::Of(Units::MetersLength(1140), Units::MetersLength(1030), Units::zero()),
             Units::MetersPerSecondSpeed(10), Units::MetersPerSecondSpeed(7.5), Units::zero(), Units::zero(),
             Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero());
   states.push_back(s0);

   // Case D in the algorithm
   // test with single-element states vector (valid, will extrapolate, see
   // AAES-633)
   valid = IMUtils::GetCrossingTime(Units::MetersLength(100), states, traj, crossingTime);
   EXPECT_TRUE(valid);
   EXPECT_NEAR(crossingTime.value(), 59, 0.01);
   interval_management::open_source::AircraftState s1;
   s1.Create(0, Units::SecondsTime(60.1),
             EarthModel::LocalPositionEnu::Of(Units::MetersLength(1099), Units::MetersLength(1000), Units::zero()),
             Units::MetersPerSecondSpeed(10), Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero(),
             Units::zero(), Units::zero(), Units::zero(), Units::zero());
   states.push_back(s1);

   interval_management::open_source::AircraftState s2;
   s2.Create(0, Units::SecondsTime(65),
   ...
   valid =
         IMUtils::GetCrossingTime(Units::MetersLength(hp.m_path_length_cumulative_meters), states, traj, crossingTime);
   EXPECT_TRUE(valid);
   ...
   // Case B in the algorithm
   valid = IMUtils::GetCrossingTime(Units::MetersLength(50), states, traj, crossingTime);
   EXPECT_TRUE(valid);
   EXPECT_NEAR(crossingTime.value(), 65, 0.01);

   // Case C in the algoirthm
   // 2 seconds after last state, should return true with an extrapolated
   // crossing time
   valid = IMUtils::GetCrossingTime(Units::MetersLength(30), states, traj, crossingTime);
   EXPECT_TRUE(valid);
...
TEST(IMUtils, calcTimeBasedExtrapolateForward) {
   ...
   const Units::Length target_x = Units::MetersLength(hp.GetXPositionMeters()) + gs * expectedTime,
                       target_y = Units::ZERO_LENGTH;
   interval_management::open_source::AircraftState target_state;
   target_state.Create(0, Units::zero(), EarthModel::LocalPositionEnu::Of(target_x, target_y, Units::zero()), gs,
                       Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero(),
                       Units::zero(), Units::zero(), Units::zero());
   ...
   IMUtils::CalculateTimeBasedExtrapolate(ownship_dtg, target_state, ownship_horizontal_traj, extrapTime, x, y, dtg);
   EXPECT_NEAR(expectedTime.value(), extrapTime.value(), tol.value());
}

TEST(IMUtils, getTimeBasedExtrapolateStateForward) {
   /*
    * Place the target behind the ownship and calculate when it will get to
    * ownship's position and what that state will be.
    */
   ...
   // Define the geometry so that expected conditions are mathematically valid
   interval_management::open_source::AircraftState ownship_state;
   ownship_state.Create(0, Units::zero(),
                        EarthModel::LocalPositionEnu::Of(Units::MetersLength(hp.GetXPositionMeters()),
                                                         Units::MetersLength(hp.GetYPositionMeters()), Units::zero()),
                        Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero(),
                        Units::zero(), Units::zero(), Units::zero(), Units::zero());
   ...
   const Units::SecondsTime timediff(-1.0);  // make them this number of seconds apart, negative means target
                                             // further away
   ...
   const Units::Length target_x = Units::MetersLength(hp.GetXPositionMeters()) + (target_dtg - ownship_dtg),
                       target_y = Units::ZERO_LENGTH;
   interval_management::open_source::AircraftState target_state;
   target_state.Create(0, Units::zero(), EarthModel::LocalPositionEnu::Of(target_x, target_y, Units::zero()), target_gs,
                       Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero(),
                       Units::zero(), Units::zero(), Units::zero());

   // get the estimated state
   Units::SecondsTime msi;
   interval_management::open_source::AircraftState actual;
   IMUtils::GetTimeBasedExtrapolateState(ownship_state, target_state, ownship_horizontal_traj, ownship_horizontal_traj,
                                         msi, actual);
...
TEST(IMUtils, calcTimeBasedExtrapolateBackward) {
   ...
   const Units::SecondsTime expectedTime = (target_dtg - ownship_dtg) / target_gs;
   const Units::Length target_x = Units::MetersLength(hp.GetXPositionMeters()) + target_gs * expectedTime,
                       target_y = Units::ZERO_LENGTH;
   interval_management::open_source::AircraftState target_state;
   target_state.Create(0, Units::zero(), EarthModel::LocalPositionEnu::Of(target_x, target_y, Units::zero()), target_gs,
                       Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero(),
...
TEST(IMUtils, GetTargetState_aaes798) {
   ...
   vector<interval_management::open_source::AircraftState> asv(5);
   ...
   asv[0].Create(5, Units::SecondsTime(1000), enu_pos, Units::FeetPerSecondSpeed(xd), Units::FeetPerSecondSpeed(yd),
                 Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero(),
   ...
   for (int i = 1; i < asv.size(); ++i) {
      ...
      enu_pos.z = Units::zero();
      asv[i].Create(asv[i - 1].GetId(), asv[i - 1].GetTimeStamp() + tstep, enu_pos, Units::FeetPerSecondSpeed(xd),
                    Units::FeetPerSecondSpeed(yd), Units::zero(), Units::zero(), Units::zero(), Units::zero(),
                    Units::zero(), Units::zero(), Units::zero(), Units::zero());
   }
   ...
   IMUtils::GetProjectedTargetState(asv, ownship_horizontal_traj,
                                    Units::SecondsTime(asv.back().GetTimeStamp().value() - 0.2), Units::Angle(),
...
TEST(IMUtils, TargetState) {
   ...
   enu_pos.y = Units::MetersLength(hp2.GetYPositionMeters()) - Units::FeetLength(0.2);
   enu_pos.z = Units::zero();
   asv[0].Create(5, Units::zero(), enu_pos, Units::FeetPerSecondSpeed(xd), Units::FeetPerSecondSpeed(yd), Units::zero(),
                 Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero(),
                 Units::zero());
   ...
   for (int i = 1; i < asv.size(); ++i) {
      ...
      enu_pos.y = asv[i - 1].GetPositionY() + Units::FeetLength(yd * tstep);
      enu_pos.z = Units::zero();
      asv[i].Create(5, asv[i - 1].GetTimeStamp() + tstep, enu_pos, Units::FeetPerSecondSpeed(xd),
                    Units::FeetPerSecondSpeed(yd), Units::zero(), Units::zero(), Units::zero(), Units::zero(),
                    Units::zero(), Units::zero(), Units::zero(), Units::zero());
   }
   ...
   // Tests
   bool built;
   interval_management::open_source::AircraftState current_ownship_state;
   current_ownship_state.Create(5, Units::zero(), enu_pos, xd, yd, Units::zero(), Units::zero(), Units::zero(),
                                Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero());

   // time-too-early, hp2 extrapolated
   interval_management::open_source::AircraftState result = IMUtils::GetProjectedTargetState(
         asv, ownship_horizontal_traj, Units::SecondsTime(asv.front().GetTimeStamp().value() - 0.5),
         current_ownship_state.GetHeadingCcwFromEastRadians(), built);
   EXPECT_TRUE(built);
   EXPECT_DOUBLE_EQ(asv.front().GetTimeStamp().value() - 0.5, result.GetTimeStamp().value());
   const Units::FeetLength expectxbefore =
         Units::MetersLength(hp2.GetXPositionMeters()) -
         xd * cos(current_ownship_state.GetHeadingCcwFromEastRadians()) * Units::SecondsTime(0.5);
   const Units::FeetLength expectybefore =
         Units::MetersLength(hp2.GetYPositionMeters()) -
         yd * sin(current_ownship_state.GetHeadingCcwFromEastRadians()) * Units::SecondsTime(0.5);
   ...
   result = IMUtils::GetProjectedTargetState(asv, ownship_horizontal_traj, asv.front().GetTimeStamp(), Units::Angle(),
                                             built);
   EXPECT_TRUE(built);
   EXPECT_DOUBLE_EQ(asv.front().GetTimeStamp().value(), result.GetTimeStamp().value());
   EXPECT_NEAR(Units::FeetLength(Units::MetersLength(hp2.GetXPositionMeters())).value(),
               Units::FeetLength(result.GetPositionX()).value(), 0.2);
   EXPECT_NEAR(Units::FeetLength(Units::MetersLength(hp2.GetYPositionMeters())).value(),
               Units::FeetLength(result.GetPositionY()).value(), 0.2);
   ...
   result = IMUtils::GetProjectedTargetState(
         asv, ownship_horizontal_traj, Units::SecondsTime(asv[0].GetTimeStamp().value() + .5), Units::Angle(), built);
   EXPECT_TRUE(built);
   ...
   result =
         IMUtils::GetProjectedTargetState(asv, ownship_horizontal_traj, asv[2].GetTimeStamp(), Units::Angle(), built);
   EXPECT_TRUE(built);
   EXPECT_DOUBLE_EQ(asv[2].GetTimeStamp().value(), result.GetTimeStamp().value());
   EXPECT_NEAR(Units::FeetLength(Units::MetersLength(hp1.GetXPositionMeters())).value(),
               Units::FeetLength(result.GetPositionX()).value(), 0.2);
   EXPECT_NEAR(Units::FeetLength(Units::MetersLength(hp1.GetYPositionMeters())).value(),
               Units::FeetLength(result.GetPositionY()).value(), 0.2);

   // time at last element, should be basically hp0
   result = IMUtils::GetProjectedTargetState(asv, ownship_horizontal_traj, asv.back().GetTimeStamp(), Units::Angle(),
                                             built);
   EXPECT_TRUE(built);
   EXPECT_DOUBLE_EQ(asv.back().GetTimeStamp().value(), result.GetTimeStamp().value());
   EXPECT_NEAR(Units::FeetLength(Units::MetersLength(hp0.GetXPositionMeters())).value(),
               Units::FeetLength(result.GetPositionX()).value(), 0.2);
   EXPECT_NEAR(Units::FeetLength(Units::MetersLength(hp0.GetYPositionMeters())).value(),
               Units::FeetLength(result.GetPositionY()).value(), 0.2);

   // time after last element, hp0 extrapolated
   result = IMUtils::GetProjectedTargetState(asv, ownship_horizontal_traj,
                                             Units::SecondsTime(asv.back().GetTimeStamp().value() + 0.5),
                                             Units::Angle(), built);
   EXPECT_FALSE(built);
...
TEST(IMUtils, GetTargetStateAtGivenDtgOnOwnshipsPath) {
   ...
   std::vector<interval_management::open_source::AircraftState> aircraft_state_history(6);
   EarthModel::LocalPositionEnu enu_pos;
   enu_pos.x = Units::MetersLength(hp1.GetXPositionMeters());
   enu_pos.y = Units::FeetLength(0.5);
   enu_pos.z = Units::zero();
   aircraft_state_history[0].Create(5, Units::zero(), enu_pos, Units::FeetPerSecondSpeed(xd), Units::zero(),
                                    Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero(),
                                    Units::zero(), Units::zero(), Units::zero());
   ...
   for (int i = 1; i < aircraft_state_history.size(); ++i) {
      ...
      aircraft_state_history[i].Create(
            aircraft_state_history[i - 1].GetId(), aircraft_state_history[i - 1].GetTimeStamp() + time_step, enu_pos,
            Units::FeetPerSecondSpeed(xd), Units::zero(), Units::zero(), Units::zero(), Units::zero(), Units::zero(),
            Units::zero(), Units::zero(), Units::zero(), Units::zero());
   ...
   // the y component of the ownship route is zero, so we expect all y values to
   // get projected to 0.
   Units::Length expected_y_value = Units::ZERO_LENGTH;

   interval_management::open_source::AircraftState current_state;
   current_state.m_x =
         Units::FeetLength(Units::MetersLength(ownship_horizontal_traj.back().GetXPositionMeters())).value();
   current_state.m_y = 0;
   ...
   Units::Length target_x = distance_before_beginning_of_route -
   ...
   result = IMUtils::GetTargetStateOnOwnshipPathForDtg(
         aircraft_state_history, ownship_horizontal_traj,
         Units::MetersLength(aircraft_state_history[2].m_distance_to_go_meters), Units::Angle());
   ...
   EXPECT_NEAR(Units::FeetLength(aircraft_state_history.back().m_x).value(), Units::FeetLength(result.m_x).value(),
               1e-10);
   EXPECT_DOUBLE_EQ(Units::FeetLength(expected_y_value).value(), Units::FeetLength(result.m_y).value());
...
TEST(IMUtils, ConvertAircraftStates) {
   ...
   const interval_management::open_source::AircraftState converted_state_2 =
         IMUtils::ConvertToIntervalManagementAircraftState(converted_state_1);
   ...
   EXPECT_DOUBLE_EQ(Units::MetersLength(actual_x).value(),
                    Units::MetersLength(converted_state_2.GetPositionX()).value());
   EXPECT_DOUBLE_EQ(Units::MetersLength(actual_y).value(),
                    Units::MetersLength(converted_state_2.GetPositionY()).value());
   EXPECT_DOUBLE_EQ(Units::MetersLength(actual_z).value(),
                    Units::MetersLength(converted_state_2.GetPositionZ()).value());
   EXPECT_DOUBLE_EQ(Units::MetersPerSecondSpeed(actual_xd).value(),
                    Units::MetersPerSecondSpeed(converted_state_2.GetSpeedXd()).value());
   EXPECT_DOUBLE_EQ(Units::MetersPerSecondSpeed(actual_yd).value(),
                    Units::MetersPerSecondSpeed(converted_state_2.GetSpeedYd()).value());
   EXPECT_DOUBLE_EQ(Units::MetersPerSecondSpeed(actual_zd).value(),
                    Units::MetersPerSecondSpeed(converted_state_2.GetSpeedZd()).value());
   EXPECT_DOUBLE_EQ(Units::RadiansAngle(actual_gamma).value(),
                    Units::RadiansAngle(converted_state_2.GetGamma()).value());
   EXPECT_DOUBLE_EQ(Units::RadiansAngle(actual_psi).value(), Units::RadiansAngle(converted_state_2.GetPsi()).value());
   EXPECT_DOUBLE_EQ(Units::CelsiusTemperature(actual_sensed_temperature).value(),
                    Units::CelsiusTemperature(converted_state_2.GetSensedTemperature()).value());
   EXPECT_DOUBLE_EQ(Units::MetersPerSecondSpeed(actual_sensed_veast).value(),
                    Units::MetersPerSecondSpeed(converted_state_2.GetSensedWindEastComponent()).value());
   EXPECT_DOUBLE_EQ(Units::MetersPerSecondSpeed(actual_sensed_vnorth).value(),
                    Units::MetersPerSecondSpeed(converted_state_2.GetSensedWindNorthComponent()).value());
   EXPECT_DOUBLE_EQ(Units::MetersPerSecondSpeed(actual_sensed_wind_parallel).value(),
                    Units::MetersPerSecondSpeed(converted_state_2.GetSensedWindParallelComponent()).value());
   EXPECT_DOUBLE_EQ(Units::MetersPerSecondSpeed(actual_sensed_wind_perpendicular).value(),
                    Units::MetersPerSecondSpeed(converted_state_2.GetSensedWindPerpendicularComponent()).value());
}
}  // namespace imalgo
}  // namespace test
}  // namespace aaesim
