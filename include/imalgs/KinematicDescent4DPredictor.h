...
#include "aaesim/VerticalPredictor.h"
#include "aaesim/KineticDescent4DPredictor.h"

class KinematicDescent4DPredictor : public VerticalPredictor
{
public:

   enum KinematicDescentType
   {
      CONSTRAINED
   };
   ...
   void SetMembers(const double &mach_descent,
                   const Units::Speed ias_descent,
                   const Units::Length cruise_altitude,
                   const Units::Length transition_altitude);

   void SetConditionsAtEndOfRoute(const Units::Length altitude_at_end_of_route,
                                  const Units::Speed ias_at_end_of_route);
...
private:
   ...
   VerticalPath ConstantCasVerticalPath(VerticalPath vertical_path,
   ...
   VerticalPath ConstantMachVerticalPath(VerticalPath vertical_path,
                                         double altitude_at_end,
                                         std::vector<HorizontalPath> &horizontal_path,
                                         std::vector<PrecalcWaypoint> &precalc_waypoints,
                                         double gamma,
   ...
   VerticalPath ConstantGeometricFpaVerticalPath(VerticalPath vertical_path,
                                                 double altitude_at_end,
   ...
   VerticalPath ConstantFpaDecelerationVerticalPath(VerticalPath vertical_path,
                                                    double altitude_at_end,
   ...
   VerticalPath LevelVerticalPath(VerticalPath vertical_path,
                                  double x_end,
                                  std::vector<HorizontalPath> &horizontal_path,
                                  const WeatherPrediction &weather_prediction,
                                  const Units::Length &aircraft_distance_to_go);

   VerticalPath ConstantDecelerationVerticalPath(VerticalPath vertical_path,
   ...
   VerticalPath LevelDecelerationVerticalPath(VerticalPath vertical_path,
                                              double deceleration,
                                              double velocity_cas_end,
                                              std::vector<HorizontalPath> &horizontal_path,
                                              const WeatherPrediction &weather_prediction,
   ...
   void ComputeWindCoefficients(Units::Length altitude,
                                Units::Angle course,
                                const WeatherPrediction &weather_prediction,
...
inline KinematicDescent4DPredictor::KinematicDescentType KinematicDescent4DPredictor::GetDescentType() const {
   return m_kinematic_descent_type;
}

inline const Units::Length KinematicDescent4DPredictor::GetAltitudeAtEndOfRoute() const {
   return m_altitude_at_end_of_route;
}

inline void KinematicDescent4DPredictor::SetConditionsAtEndOfRoute(const Units::Length altitude_at_end_of_route,
                                                                   const Units::Speed ias_at_end_of_route) {
   m_altitude_at_end_of_route = altitude_at_end_of_route;
   m_ias_at_end_of_route = ias_at_end_of_route;
}

inline const double KinematicDescent4DPredictor::GetDecelerationRateFPA() const {
